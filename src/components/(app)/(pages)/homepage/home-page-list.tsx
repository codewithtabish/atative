      ...
      <div className="min-w-0 flex-1">
        ...
        <h4
          className={`mt-1 font-semibold leading-snug transition-colors group-hover:text-primary ${
            large ? "text-lg line-clamp-2" : "text-sm line-clamp-2"
          }`}
        >
          {blog.title}
        </h4>
...
export default async function HomeBlog() {
  ...
  // Smart distribution for only 2 blogs (or more later)
  const hero = blogs[0] ?? null;
  const secondary = blogs[1] ?? null;
  const rest = blogs.slice(2);

  // For the “Latest” sidebar we reuse what we have
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="border-y border-border py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {/* Empty state cards when < 4 blogs */}
            {blogs.length < 4 &&
        ...
        <section className="border-t border-border py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-12 lg:grid-cols-2">
              {/* Left column – first blog as large */}
              <div>
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <ol className="space-y-5">
            {blogs.slice(0, 5).map((blog, index) => (
              <li key={blog.id}>
                <Link href={`/blog/${blog.slug}`} className="group flex items-start gap-5">
      ...
      <footer className="border-t border-border py-10 text-center">
        <p className="text-sm font-semibold tracking-tight">ATIVE</p>
